...
bool TextContainer::createText(const std::string &text, const Color &color, int32_t font_id, std::int32_t &out_text_id) {
	...
	out_text_id = static_cast<std::int32_t>(m_Text.add(texture));
    return true;
}

bool TextContainer::reloadText(const std::string &text, const Color &color, int32_t font_id, std::int32_t out_text_id) {
	auto font = m_Font.get(font_id);
	if(nullptr == font) {
    ...
    return m_Text.replace(static_cast<std::size_t>(out_text_id), texture);
}
